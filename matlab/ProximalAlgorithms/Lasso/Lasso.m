...
x0 = sprandn(n,1,0.05); % optimal variable, with sparsity pattern
A = randn(m,n); % randomly initialize A
A = A*spdiags(1./sqrt(sum(A.^2))',0,n,n); % normalize columns
v = sqrt(0.001)*randn(m,1); % v is vector of random noise
b = A*x0 + v;
...
for k = 1:MAX_ITER
    while 1
        ...
        % then line search to find step size
...
%%%%%%%%%%%%%% solve using fast proximal gradient %%%%%%%%%%%%%%%%
lambda = 1;
...
xprev = x;
for k = 1:MAX_ITER
    % initialize the start point using a momentum
    y = x + (k/(k+3))*(x - xprev);
    while 1
        grad_y = AtA*y - Atb;
